# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 22:32:52 2024
...
def cross_validate_model(model, X_train, y_train, params, n_splits=5, num_epochs=1000):
    ...
    for fold, (train_ind, valid_ind) in enumerate(cv.split(X_train)):
        ...
        for epoch in range(num_epochs):
            # Adjust learning rate at epoch 500
            if epoch == 500:
                for param_group in optimizer.param_groups:
                    param_group['lr'] = 0.0001
                print("Learning rate adjusted to 0.0001")
...
def plot_loss_curves(save_path='loss_curve.png', plot_folds=False):
    ...
    # Add vertical line at epoch 500 to show learning rate change
    plt.axvline(x=500, color='r', linestyle='--', alpha=0.5, label='LR change')
    
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
    plt.title('Training and Validation Loss Curve + Variable Learning Rate')
    plt.legend()
    plt.show()
