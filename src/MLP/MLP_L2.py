# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 19:46:53 2024
...
def cross_validate_model(model, X_train, y_train, params, n_splits=5, num_epochs=1000):
    ...
    for fold, (train_ind, valid_ind) in enumerate(cv.split(X_train)):
        ...
        model = Net(**params)
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001)  # Removed weight_decay since we're using explicit L2
        train_losses = []
        val_losses = []
        ...
        for epoch in range(num_epochs):
            ...
            train_loss = RMSELoss(outputs, y_fold_train.unsqueeze(1), model)
            ...
            val_loss = RMSELoss(val_outputs, y_val.unsqueeze(1), model)
...
def plot_loss_curves(save_path='loss_curve.png', plot_folds=False):
    ...
    plt.grid(True, linestyle='--', alpha=0.7)  # Dashed lines with slight transparency
    if plot_folds:
    ...
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
    plt.title('Training and Validation Loss Curve + L2 Regularisation')
    plt.legend()
    plt.show()
...
plot_loss_curves(plot_folds=False)
