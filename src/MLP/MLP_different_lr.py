# -*- coding: utf-8 -*-
"""
Created on Wed Nov 20 00:03:27 2024
...
for lr in learning_rates:
   ...
   for epoch in range(1000):
       ...
       train_loss = RMSELoss(outputs, y_train_tensor.unsqueeze(1), model)
       ...
       with torch.no_grad():
           ...
           val_loss = RMSELoss(val_outputs, y_val_tensor.unsqueeze(1), model)
       
       train_losses.append(float(train_loss.item()))
       val_losses.append(float(val_loss.item()))
       
       if (epoch+1) % 100 == 0:
           print(f"LR: {lr}, Epoch [{epoch+1}/1000], Train RMSE: {train_loss.item():.4f}, Val RMSE: {val_loss.item():.4f}")
...
for lr in learning_rates:
   plt.plot(all_train_losses[f'lr={lr}'], label=f'Train (lr={lr})')
   plt.plot(all_val_losses[f'lr={lr}'], label=f'Val (lr={lr})')
plt.xlabel('Epoch')
...
plt.figure(figsize=(12, 6))
for lr in learning_rates:
    plt.plot(all_val_losses[f'lr={lr}'], label=f'lr={lr}')
plt.xlabel('Epoch')
plt.ylabel('RMSE Loss')
...
# Save losses
with open('losses_comparison.json', 'w') as f:
   json.dump({
       'train_losses': all_train_losses,
       'val_losses': all_val_losses
   }, f, indent=2)

# Use best learning rate based on validation loss
best_lr = min(all_val_losses.items(), key=lambda x: min(x[1]))[0]
print(f"\nBest learning rate based on validation: {best_lr}")
