# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 23:43:17 2024
...
def cross_validate_ridge(X, y, alpha=1, n_splits=5):
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=42)
    global global_train_scores, global_val_scores
    
    global_train_scores = []  # Reset scores
    global_val_scores = []
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        global_train_scores.append(train_rmse)
        global_val_scores.append(val_rmse)
        
        print(f"Fold {fold+1}: Train RMSE = {train_rmse:.4f}, Val RMSE = {val_rmse:.4f}")
    
    return np.mean(global_train_scores), np.mean(global_val_scores)
...
mean_train_rmse, mean_val_rmse = cross_validate_ridge(X, y)
print(f"\nAverage Train RMSE: {mean_train_rmse:.4f}")
print(f"Average Validation RMSE: {mean_val_rmse:.4f}")

# Train final model and make predictions
final_model = Ridge(alpha=1)
final_model.fit(X, y)
test_predictions = final_model.predict(encoded_dt.values)

test_df = pd.DataFrame({
    'id': range(188533, 188533 + len(test_predictions)),
    'price': test_predictions
})
test_df.to_csv('ridge_regression_predictions.csv', index=False)
