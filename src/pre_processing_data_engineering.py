# -*- coding: utf-8 -*-
"""
Created on Thu Nov 14 22:16:02 2024
...
def cross_validate_model(model, X_train, y_train, params, n_splits=5, num_epochs=1000):
    ...
    test_preds = np.zeros((len(X_tensor), n_splits), dtype=np.float32)
    all_train_losses = []
    all_val_losses = []
    ...
    for fold, (train_ind, valid_ind) in enumerate(cv.split(X_train)):
        ...
        test_preds[:, fold] = model(X_tensor).squeeze().detach().numpy()
    ...
    plt.plot(avg_train_loss, label='Training RMSE')
    plt.plot(avg_val_loss, label='Validation RMSE')
    plt.xlabel('Epoch')
    plt.ylabel('RMSE')
...
model, test_preds = cross_validate_model(Net, X_tensor, y_tensor, params={'input_size': X_tensor.shape[1]}, num_epochs=1000)
...
def train_model(model, X_train, y_train, X_val, y_val, num_epochs=2000):
...
# Save the trained model
torch.save(model.state_dict(), 'price_prediction_model_181124_nn.pth')
